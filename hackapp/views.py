...
def patient3(request):
    ...
    def predict_disease_from_symptom(symptom_list):
     ...
     # Load pre-trained model
        clf = load(str("hackapp/saved_modeldecision_tree.joblib"))
    ...
    # Cleanup
        ...
        return f"{result[0]}"
    symptom_list = ['itching', 'skin_rash']
    result = predict_disease_from_symptom(symptom_list)
    print(result)
    return render(request, 'patient3.html')
def my_def_in_view(request):
    result = request.GET.get('symp_list_send', None)
